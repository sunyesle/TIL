...
CAS 연산은 다음과 같이 동작한다.
- **비교**(Compare): CPU가 특정 메모리 주소의 실제 값과 기댓값을 비교한다.
- **교환**(Swap):
    - **일치하는 경우** 메모리 값을 새로운 값으로 교체하고, 성공 플래그를 세팅한다.
    - **일치하지 않는 경우** 메모리 값을 변경하지 않고, 실패 플래그를 세팅한다.

이 모든 과정은 CPU 명령어 수준에서 더 이상 쪼개질 수 없는 단일 원자 연산으로 수행된다.
