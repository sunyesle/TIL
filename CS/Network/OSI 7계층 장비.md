...
## L4 | 전송 계층 | Transport
> 패킷의 IP와 포트 번호를 기반으로 트래픽을 분산한다.

### L4 스위치
TCP/IP 프로토콜을 기반으로 동작한다. 주로 로드 밸런싱(부하 분산)에 사용된다.

- **분산 기준**: 패킷의 IP 주소와 포트 번호를 기반으로 트래픽을 분산한다.
- **특징**: 패킷 내용을 열어보지 않기 때문에 처리가 매우 빠르다.

## L7 | 애플리케이션 계층 | Application
> URL, HTTP 헤더, 쿠키 등 애플리케이션 레벨의 데이터를 기반으로 트래픽을 분산한다.

### L7 스위치
HTTP, FTP 등 애플리케이션 레벨에서의 데이터 처리를 지원한다. 주로 로드 밸런싱에 사용된다.

- **분산 기준**: IP 주소와 포트 번호 외에도, URL, HTTP 헤더, 쿠키 등 패킷 내용을 기반으로 요청을 분산한다.
- **특징**: 패킷 내용을 세부적으로 분석하므로 `/images` 같은 특정 경로에 따라 서로 다른 서버로 요청을 보내는 콘텐츠 기반 라우팅이 가능하다.
