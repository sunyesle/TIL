...
Foo fooXml = xmlFactory.getBean(Foo.class);

BeanFactory javaFactory = new AnnotationConfigApplicationContext(Config.class);
Foo fooJava = xmlFactory.getBean(Foo.class);
```
클라이언트에서는 BeanFactory 인터페이스를 통해 빈을 요청하며 구체적인 생성 방식에 대해 알 필요가 없다.
빈 생성의 구체적인 로직은 서브 클래스에 위임한다. (XML 파일 기반, 어노테이션 기반 등)
