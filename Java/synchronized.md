...
## 고유락(Intrinsic Lock)
Java의 **모든 객체**(**인스턴스, 클래스**)는 내부적으로 락을 가지고 있다. 이를 모니터, 모니터 락, 뮤텍스라고도 한다. **고유락**은 Java의 동기화 방법 중 하나로, **객체 단위로 동기화를 제어**하는 데 사용된다.
...
## synchronized 키워드
`synchronized` 키워드를 사용하여 블록 단위로 락을 획득하고 해제할 수 있으며, 이를 통해 스레드 간의 동기화를 구현할 수 있다. 해당 키워드를 사용하면 지정한 객체에 암시적인 락이 걸리며, 블록을 진입할 때 락을 획득하고 블록을 벗어날 때 해제된다. JVM이 자동으로 락의 획득과 해제를 처리하기 때문에 프로그래머는 락을 명시적으로 관리할 필요가 없다.
...
### 제한 사항
간단한 동기화에는 synchronized를 사용하는 것이 더 효율적일 수 있으나 세밀한 제어가 힘들다. 더 세밀한 동시성 제어가 필요한 경우 `java.util.concurrent` 패키지를 활용하자.
