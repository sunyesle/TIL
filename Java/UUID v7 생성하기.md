...
1. **데이터베이스 기본 키**
    ...
    - 무작위 값으로 인해 다음 ID를 예측하기 어려워 보안에 유리하다.
2. **로그 및 이벤트 추적**
    - 타임스탬프가 포함되어 있어 ID 값만으로 언제 발생한 이벤트인지 파악할 수 있고, 특정 시간대의 로그를 정렬 및 필터링 할 수 있다.

## 예시
현재 Java 표준 라이브러리인 `java.util.UUID`는 UUID v7 생성을 지원하지 않는다.
...
void main() {
    ...
    UUID uuidV7_2 = Generators.timeBasedEpochRandomGenerator().generate(); // Version 7 with per-call random values
}
```
