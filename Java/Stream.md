...
스트림은 "**스트림 생성 → 스트림 가공 → 결과 생성**"의 구조로 구성되어 있다.

스트림을 사용하면 람다식과 메서드 체이닝을 활용해 데이터 처리 흐름을 명확하고 간결하게 표현할 수 있다.

## 예제
Stream API의 기능을 알아보기 앞서, 간단한 예제를 통해 기존방식과 스트림을 이용한 방식을 비교해 보자.
...
### Slicing
`takeWhile()`은 `Predicate`가 `false`를 반환할 때까지의 요소를 취한다.
```java
...
`dropWhile()`은 `Predicate`가 `false`를 반환할 때까지의 요소를 모두 버리고 나머지 요소를 반환한다.
```java
IntStream.of(1, 3, 5, 7, 9)
        .dropWhile(i -> i < 5);
// [5, 7, 9]
...
List<Student> students = Arrays.asList(
        new Student(95, 70, 80),
        new Student(80, 55, 96),
        new Student(64, 90, 90));
...
최소, 최대, 평균은 스트림이 비어있는 경우 표현할 수 없기 때문에 `Optional`을 이용해 리턴한다.
```java
...
### Collecting 
`collect()`는 스트림의 요소를 컬렉션, 리스트, 세트 또는 맵과 같은 다른 형태로 변환하는데 사용된다.<br>
...
#### Collectors.joining()
스트림에서 작업한 결과를 하나의 `String`으로 이어 붙여 반환한다.
```java
...
`IntStream`으로 바꿔주는 `mapToInt()` 메서드를 사용해서 더 간단하게 표현할 수 있다.
```java
...
`collect()` 결과를 대상으로 추가 작업을 진행할 수 있다.
```java
...
스트림 계산 결과를 보고할 때(print)만 사용하는 것이 좋다.<br>
로직이 들어가 있다면 `forEach()`가 아니라 스트림의 중간 연산(`filter()`, `map()`)을 활용하거나, 반복문을 사용하는 것이 올바른 방향이다.
