...
### 시퀀스보다 좋은 솔루션
한 개 이상의 구분 속성과 함께 뒤쪽에 순번 대신 입력일시를 두는 방식으로 PK구조를 설계하면 채번 또는 INSERT 과정에서 생기는 Lock 이슈를 거의 해소할 수 있다.

입력일시를 PK에 포함하면 파티션 키로 바로 활용 가능하고, 오래된 데이터를 삭제/아카이빙 할 때도 유리하다.

### 인덱스 블록 경합
채번 테이블 로우 Lock이나 시퀀스 Lock이 병목지점이면 잘 안 나타나지만, 입력일시나 MAX+1을 사용하여 INSERT 성능이 빨라지는 경우 인덱스 블록 경합이 나타나기 시작한다.

**인덱스 경합은 Right Growing 인덱스에서 가장 흔히 볼 수 있다.**

일련번호나 입력일시처럼 순차적으로 값이 증가하는 단일컬럼 인덱스는 항상 맨 우측 블록에만 데이터가 입력된다.
같은 블록을 갱신하려는 프로세스 간 버퍼 Lock 경합이 발생할 수 있으며, 동시 INSERT가 많을 때 트랜잭션 성능을 떨어뜨리는 주범이다.

구분 속성을 앞에두면 Right Growing 인덱스는 아니지만 동시성이 높을경우 경합은 생길 수 있다.
구분 속성의 값 종류 개수가 적을수록 경합도 심하다.

인덱스 블록 경합을 해소하는 가장 일반적인 방법을 **인덱스를 해시 파티셔닝**하는 것이다.
또는 인덱스를 **리버스 키 인덱스로 전환**하는 방법도 고려할 수 있다.
