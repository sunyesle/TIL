...
## 이력 조회
<img width="301" height="136" alt="Image" src="https://github.com/user-attachments/assets/862406d6-3ae5-4f21-a687-242e6ffd1424" /><br>
상태변경이력 테이블 인덱스 구성: [`장비번호 + 변경일자 + 변경순번`]

### 이력 테이블 단일 컬럼 조회
스칼라 서브쿼리 부분에서 First Row Stopkey 알고리즘이 작동한다.
```sql
...
### 이력 테이블 다수의 컬럼 조회
**◆ 스칼라 서브쿼리**<br>
쿼리가 복잡하고 상태변경이력을 3번 조회하는 비효율은 있지만, First Row Stopkey 알고리즘이 작동하므로 성능은 비교적 좋다.
```sql
SELECT 장비번호, 장비명, 상태코드
     , (SELECT MAX(H.변경일자)
        FROM 상태변경이력 H
...
**◆ INDEX_DESC 힌트를 활용**<br>
이력 테이블에서 읽어야 하는 컬럼이 더 많은 경우 INDEX_DESC 힌트를 활용하여 다음과 같이 쿼리를 작성할 수 있다.
하지만, 인덱스 구성이 변경되었을 때 결과집합이 달라질 수 있다.
```sql
SELECT 장비번호, 장비명
...
**◆ 11g 신기능 활용**<br>
Predicate Pushing이라고 불리는 쿼리 변환이 작동하여, `장비번호 = P.장비번호` 조건절이 인라인 뷰 안쪽으로 파고 들어가 Top N Stopkey 알고리즘이 작동한다.
인덱스 구성이 변경됐을 때 성능이 느려질 수는 있지만, 쿼리 결과집합은 보장된다.
```sql
SELECT 장비번호, 장비명
     , SUBSTR(최종이력, 1, 8) 최종변경일자
     , TO_NUMBER(SUBSTR(최종이력, 9, 4)) 최종변경순번
...
**◆ 12c 신기능 활용**<br>
11g까지는 ORA-00904 오류가 발생했던 패턴이다.
12c에서는 SQL 파싱 오류 없이 Top N Stopkey 알고리즘이 잘 작동한다.
```sql
SELECT 장비번호, 장비명
     , SUBSTR(최종이력, 1, 8) 최종변경일자
     , TO_NUMBER(SUBSTR(최종이력, 9, 4)) 최종변경순번
...
### 대량의 이력 조회
대량의 이력을 조회할 때는 인덱스를 이용한 Stopkey 기능 작동 여부가 튜닝의 핵심 요소가 아니다.
인덱스 활용 패턴은 랜덤 I/O 발생량만큼 성능도 비례해서 느려지므로 대량 데이터 조회에 좋은 솔루션이 되지 못한다.
...
FROM 장비 P
     , (SELECT 장비번호, 변경일자, 변경순번, 상태코드
             , ROW_NUMBER() OVER (PARTITION BY 장비번호
                                  ORDER BY 변경일자 DESC, 변경순번 DESC) ROW_NUMBER
        FROM 상태변경이력) H
WHERE H.장비번호 = P.장비번호
AND H.RNUM = 1
```
...
### 선분이력 모델
<img width="301" height="151" alt="Image" src="https://github.com/user-attachments/assets/a0898a8a-3367-4c43-a459-5e179f298083" />

업무 특성에 따라 선분이력 모델도 고려할 만하다.<br>
다음과 같이 쉽게 이력을 조회할 수 있고, 쿼리가 간단한 만큼 성능 측면에 이점도 생긴다.
```sql
SELECT P.장비번호, P.장비명
...
다양한 이력 모델의 장단점을 파악한 뒤, 각 상황에 적합한 모델을 선택하는 것이 중요하다.

## Sort Group By 생략
그룹핑 연산에도 인덱스를 활용할 수 있다.

GROUP BY 절에 포함된 모든 컬럼이 인덱스의 선두부터 포함되어 있다면, Sort Group By 연산을 생략할 수 있다.<br>
실행계획에는 Sort Group By Nosort라고 표시된다.<br>
인덱스 순서대로 읽으며 바로 그룹핑이 가능하기 때문에, 부분범위 처리가 가능하다.
